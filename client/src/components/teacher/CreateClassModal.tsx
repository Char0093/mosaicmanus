...
export default function CreateClassModal({ open, onOpenChange, onCreated }: Props) {
  ...
  useEffect(() => { if (open) { setStep(1); setError(""); setPreviewCode(newCode()); } }, [open]);
  ...
  const submit = () => {
    setError("");
    createClass.mutate({ name: name.trim(), subject, yearLevel, topics, description: description.trim() || undefined }, { onSuccess: (created) => { toast.success(`Class created! Your kiosk code is ${created.kioskCode}`); onCreated(created); onOpenChange(false); }, onError: (cause) => setError(cause.message || "Could not create this class. Please try again.") });
  };
