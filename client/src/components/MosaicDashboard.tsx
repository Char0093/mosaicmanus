...
import { BarChart3, BellRing, BookOpenCheck, ChevronRight, CircleHelp, Download, Grid2X2, Lightbulb, Radio, RefreshCw, Sparkles, UsersRound, Wifi, X } from "lucide-react";
...
function Heatmap({ learners }: { learners: Learner[] }) {
  const concepts = ["Mass vs. weight", "Force & motion", "Falling objects", "Measurement"];
  const cells = learners.slice(0, 12);
  return <section className="panel heatmap-panel"><div className="panel-heading"><div><div className="eyebrow">Concept diagnostic</div><h2>Where thinking gets stuck</h2></div><button className="text-button">See responses <ChevronRight size={15} /></button></div><div className="heatmap-scroll"><div className="heatmap"><div className="heatmap-row heatmap-row--head"><span>Learner</span>{concepts.map((concept) => <span key={concept}>{concept}</span>)}</div>{cells.map((learner, index) => <div className="heatmap-row" key={learner.id}><span className="learner-label"><b>{learner.initials}</b>{learner.name}</span>{concepts.map((concept, cellIndex) => { const hot = learner.misconception?.toLowerCase().includes("mass") && cellIndex === 0; const level = hot ? "hot" : (index + cellIndex * 2) % 5 === 0 ? "watch" : "clear"; return <span className={`heat-cell heat-cell--${level}`} key={concept} aria-label={`${learner.name} ${concept} ${level}`} />; })}</div>)}</div></div><div className="heat-legend"><span><i className="heat-cell heat-cell--clear" />Secure</span><span><i className="heat-cell heat-cell--watch" />Watch</span><span><i className="heat-cell heat-cell--hot" />Needs intervention</span></div></section>;
}
...
export default function MosaicDashboard() {
  ...
  const renderMain = () => {
    ...
    if (view === "heatmap") return <><div className="section-title"><div><div className="eyebrow">Live class view</div><h1>Concept signals</h1><p>Patterns appear here as learners complete missions and pulse checks.</p></div></div><Heatmap learners={data.learners} /></>;
